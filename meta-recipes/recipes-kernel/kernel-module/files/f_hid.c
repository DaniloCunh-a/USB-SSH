...
#include "g_tpd.h" //Incluido pelo cliente
...
// u32 __iomem usgotgsc_addr; //comentei pq eu não sei oq eh

static struct usb_composite_overwrite coverwrite;

/*
//Eu realmnete não sei
struct usb_udc {
...
struct f_hidg {
	...
	unsigned char			protocol; //Add para compatibilização com Novo kernel
	unsigned char			idle; //Add para compatibilização com Novo kernel
	unsigned short			report_desc_length;
	char				*report_desc;
	unsigned short			report_length;

	/***********Para verificação de compatibilidade**********************/
	/*
	...
	/* recv report */
	spinlock_t			read_spinlock; //same read_spinlock
	...
	/* send report */
	spinlock_t			write_spinlock; // nova maniera de controlar a trava mutex
	// struct mutex			lock; // provavelemente a maniera como controlamos a trava mutex mudou
	bool				write_pending;
...
static struct usb_interface_descriptor hidg_interface_desc = {
	...
	.bAlternateSetting	= 0,
	.bNumEndpoints		= 2, //Provavelmente são para HS suporte e FS suporte. 
...
static struct hid_descriptor hidg_desc = {
	...
	.bcdHID				= cpu_to_le16(0x0101), //adiconando cpu_to_le16()
	.bCountryCode			= 0x00,
	.bNumDescriptors		= 0x1,
...
//Anteriormente era *hidg_hs_descriptors, passou a ser:
...
//Anteriormente era *hidg_fs_descriptors, passou a ser:
...
//Aqui ele seleciona qual função read usar intout ou ssreport, a decisão é baseada no valor de use_out_ep
...
#define BUFF_SIZE	100
	...
	u32 i_g_pid = argp;
	int len;
	unsigned char buf_rec[BUFF_SIZE];
	u16 IDP = hid_driver->dev->idProduct;
	...
	switch (cmd)
	{
		case GIIOC_CONNECT:
			#ifdef DEBUG_TPD
				printk("Executando o GIIOC_CONNECT\n");
			#endif
...
//				usb_gadget_vbus_connect (hidg->func.config->cdev->gadget);
		...
		case GIIOC_DISCONNECT:
			#ifdef DEBUG_TPD
				printk("Execultando o GIIOC_DISCONNECT\n");
			#endif
...
//				usb_gadget_vbus_disconnect(hidg->func.config->cdev->gadget);
		...
		case GIIOC_SETSN:
			...
			// len = strlen((char __user *)argp);
			len = 11;
			copy_from_user(buf_rec, (char __user *)argp, BUFF_SIZE);
			...
			memcpy(hid_driver->strings[0]->strings[USB_GADGET_SERIAL_IDX].s, buf_rec, 11);    // Set new SN
		...
		case GIIOC_GETSTATUS:
			...
			#ifdef DEBUG_TPD
				printk(KERN_INFO "** GETSTATUS d state=0x%X\n", reg);
			#endif
                                    
			status = (reg & 0xff); //0000 1000 & 1111 1111 = 0000 1000 | 0001 0000 = 18
			
			if (atomic_read(&mdev->connected))
			{
				#ifdef DEBUG_TPD
					printk("*******Atomic read ta commdev conectado*******\n");
				#endif
			...
			#endif
			if (copy_to_user((int __user *)argp, &status, sizeof(int)) != 0) {
				
				#ifdef DEBUG_TPD
					printk("*******Falha ao escrever o valor de Reg para o user*******\n");
				#endif

        		return -EFAULT; // Falha ao escrever no espaço de usuário
    		}
			break;			

		case GIIOC_GETENDPOINTLEN:
			int report_length = hidg->report_length;
			
			#ifdef DEBUG_TPD
				printk(KERN_INFO "** GETENDPOINTLEN = %d\n", report_length);
			#endif
			
			if (copy_to_user((int __user *)argp, &report_length, sizeof(int)) != 0) {
				
				#ifdef DEBUG_TPD
					printk("*******Falha ao escrever o valor de report_lenght para o user*******\n");
				#endif
...
Adiconei intout_complete e ssreport_complete
...
static int hidg_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	// hidg_interface_desc.bNumEndpoints = hidg->use_out_ep ? 2 : 1; o valor foi atribuido com 2.
